...
function CalendarInput({
  ...
  const { styles, attributes } = usePopper(referenceElement, container, {
  ...
  const isoValue = useMemo(
    () =>
      isStrictlyValid(value, format)
        ? dayjs(value, format).toISOString()
        : null,
    [value]
  );
  ...
  const documentClick = useCallback(
    (e) => {
      if (
        ...
        !(container as any)?.contains(e.target)
      ) {
        setOpen(false);
      }
    },
...
export function DateInput(props: any) {
  const { placeholder, control, name, rules, format, inputFormat, disabled } =
    props;
  return (
    <Controller
      render={({ field }) => (
...
export function CalendarInputExample() {
  ...
  return (
    <div className="max-w-md">
      ...
      <CalendarInput
        value={ddmmyyyy}
        onChange={(v) => setDdmmyyyy(v)}
        format="DD.MM.YYYY"
      />
      <h3>With YYYY-MM-DD: {yyyymmdd}</h3>
      <CalendarInput
        value={yyyymmdd}
        onChange={(v) => setYyyymmdd(v)}
        format="YYYY-MM-DD"
      />
    </div>
  );
